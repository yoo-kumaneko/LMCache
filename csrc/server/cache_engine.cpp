...
// Kernel entry point from mem_kernels.cuh (compiled via mem_kernels.cu)
...
std::pair<std::vector<uint8_t>, bool> CacheEngine::store(
  ...
  // Set device and stream guards — ensures all ATen ops + CUDA calls
  // go to the correct device and stream (matching Python's
  // `with torch.cuda.device(...), torch.cuda.stream(...):`)
  ...
  // Transfer: GPU → tmp_buffer → L1 slab (via the CUDA kernel)
  {
    ...
    for (size_t idx = 0; idx < obj_keys.size(); ++idx) {
      ...
      int start_tok = static_cast<int>(idx) * chunk_size_;
      int end_tok = start_tok + chunk_size_;
      at::Tensor slot_slice = slot_mapping.slice(0, start_tok, end_tok);

      at::Tensor tmp_buf = gpu_ctx->get_tmp_gpu_buffer(chunk_size_);

      // Sync before kernel launch to catch async errors early
      cudaError_t pre_err = cudaGetLastError();
      if (pre_err != cudaSuccess) {
        std::fprintf(stderr,
                     "WARNING: pre-existing CUDA error on device %d: %s\n",
                     gpu_ctx->device_index(), cudaGetErrorString(pre_err));
      }

      // Debug: log kernel parameters
      std::fprintf(
          stderr,
          "[store] device=%d chunk=%zu/%zu tmp_buf=[%ld,%ld,%ld,%ld] dtype=%d "
          "kv_ptrs=[%ld] slot=[%ld] page_buf_size=%d format=%d bs=%d\n",
          gpu_ctx->device_index(), idx, obj_keys.size(), tmp_buf.size(0),
          tmp_buf.size(1), tmp_buf.size(2), tmp_buf.size(3),
          (int)tmp_buf.scalar_type(), gpu_ctx->kv_pointers().size(0),
          slot_slice.size(0), gpu_ctx->block_size() * gpu_ctx->num_blocks(),
          gpu_ctx->gpu_kv_format(), gpu_ctx->block_size());

      // GPU KV cache → tmp_buffer (D2H staging)
      multi_layer_kv_transfer(
          tmp_buf, gpu_ctx->kv_pointers(), slot_slice,
          at::Device(at::kCUDA, gpu_ctx->device_index()),
          gpu_ctx->block_size() * gpu_ctx->num_blocks(), TransferDirection::D2H,
          static_cast<GPUKVFormat>(gpu_ctx->gpu_kv_format()),
          gpu_ctx->block_size(), 0);
...
std::pair<std::vector<uint8_t>, bool> CacheEngine::retrieve(
  ...
  // Wait for async prefetch if SYNC_LOOKUP left pending state
  {
    std::lock_guard<std::mutex> lk(pending_lookups_lock_);
    auto pit = pending_lookups_.find(key.request_id);
    if (pit != pending_lookups_.end()) {
      auto& pending = pit->second;
      if (pending.has_future) {
        try {
          pending.prefetch_future.get();
        } catch (...) {
        }
      }
      pending.workers_remaining--;
  ...
  // Transfer: L1 slab → tmp_buffer → GPU KV cache
  {
    std::lock_guard<std::mutex> lk(gpu_ctx->transfer_lock());
    at::Tensor slot_mapping = gpu_ctx->get_slot_mapping_tensor(gpu_block_ids);
    ...
    for (size_t idx = 0; idx < obj_keys.size(); ++idx) {
      ...
      int chunk_start = static_cast<int>(idx) * chunk_size_;
      int chunk_end = chunk_start + chunk_size_;

      int effective_start = std::max(chunk_start, skip_first_n_tokens);
      if (effective_start >= chunk_end) continue;

      int skip_in_chunk =
          std::max(0, std::min(effective_start - chunk_start, chunk_size_ - 1));

      at::Tensor slot_slice = slot_mapping.slice(0, chunk_start, chunk_end);
      at::Tensor tmp_buf = gpu_ctx->get_tmp_gpu_buffer(chunk_size_);

      // L1 slab → tmp_buffer (H2D)
      auto& slab_ref = it->second;
      cudaMemcpyAsync(tmp_buf.data_ptr(), slab_ref.data, slab_ref.size_bytes,
                      cudaMemcpyHostToDevice, gpu_ctx->high_priority_stream());

      // tmp_buffer → GPU KV cache
      multi_layer_kv_transfer(
          tmp_buf, gpu_ctx->kv_pointers(), slot_slice,
          at::Device(at::kCUDA, gpu_ctx->device_index()),
          gpu_ctx->block_size() * gpu_ctx->num_blocks(), TransferDirection::H2D,
          static_cast<GPUKVFormat>(gpu_ctx->gpu_kv_format()),
          gpu_ctx->block_size(), skip_in_chunk);
    }
  }
...
int CacheEngine::sync_lookup(const IPCCacheEngineKey& key, int tp_size) {
  ...
  if (layout.shapes.empty()) {
    std::fprintf(stderr,
                 "ERROR: sync_lookup: no GPU context for model=%s ws=%d\n",
                 key.model_name.c_str(), key.world_size);
    return 0;
  }
  ...
  auto chunk_hashes = token_hasher_.compute_chunk_hashes(key.token_ids);
  if (chunk_hashes.empty()) return 0;
  ...
  int found_count = static_cast<int>(hit_count) / key.world_size;

  // Only save pending state if there are hits for RETRIEVE to use
  if (found_count > 0) {
    // Reserve read locks for the hit prefix
    std::vector<ObjectKey> hit_keys(obj_keys.begin(),
                                    obj_keys.begin() + hit_count);
    ...
    pending_lookups_[key.request_id] = PendingLookupState{
        key.world_size,
        key.world_size,
        {},     // no L2 future for now (L2 integration TBD)
        false,  // has_future = false
    };
  }

  std::fprintf(stderr,
               "SYNC_LOOKUP[%s]: hit_count=%ld, found_count=%d, ws=%d\n",
               key.request_id.c_str(), hit_count, found_count, key.world_size);
...
void CacheEngine::free_lookup_locks(const IPCCacheEngineKey& key, int tp_size) {
  // Handle pending lookup state from SYNC_LOOKUP
  {
    std::lock_guard<std::mutex> lk(pending_lookups_lock_);
    auto pit = pending_lookups_.find(key.request_id);
    if (pit != pending_lookups_.end()) {
