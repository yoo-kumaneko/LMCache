...
// ============================================================================
// PendingLookupState — async prefetch handoff between SYNC_LOOKUP and RETRIEVE
// ============================================================================

struct PendingLookupState {
  int world_size;
  int workers_remaining;  // Countdown; last worker removes the entry
  std::shared_future<int> prefetch_future;  // {} when no L2 results to load
  bool has_future;  // true when prefetch_future is valid
};
...
class CacheEngine {
 public:
  ...
  /// Synchronous prefix lookup with async L2 prefetch.
  /// Returns the number of matched chunks.
  int sync_lookup(const IPCCacheEngineKey& key, int tp_size);
 ...
 private:
  ...
  // Pending lookup state: request_id → state
  std::mutex pending_lookups_lock_;
  std::unordered_map<std::string, PendingLookupState> pending_lookups_;
