...
// GPUKVFormat enum from mem_kernels.cuh — replicated here to avoid pulling in
// the full CUDA kernel header (which requires __device__ compilation).
// Values must stay in sync with csrc/mem_kernels.cuh.
namespace {
...
///
/// In the C++ server, kv_caches arrives as a flat vector of CudaIpcTensorDesc
/// (list depth = 1). We inspect tensor_dim and leading shape dimensions to
/// determine the format, exactly as the Python discover_gpu_kv_format does for
/// EngineType.VLLM with list_depth == 1.
...
GPUContext::GPUContext(const std::vector<CudaIpcTensorDesc>& kv_cache_descs,
  ...
  switch (fmt) {
    case GPUKVFormat::NL_X_TWO_NB_BS_NH_HS:
      ...
      hidden_dim_size_ = static_cast<int>(shape0[3] * shape0[4]);  // NH * HS
      break;
    ...
    case GPUKVFormat::NL_X_NB_BS_HS:
      ...
      hidden_dim_size_ = static_cast<int>(shape0[2]);  // HS
      break;
  ...
  std::fprintf(stderr,
               ...
               "blocks=%d, block_size=%d, hidden_dim=%d\n",
               device_index_, gpu_kv_format_, is_mla_ ? 1 : 0, num_layers_,
               num_blocks_, block_size_, hidden_dim_size_);
  ...
  {
    auto buf_shape = get_kv_buffer_shape(chunk_size);
    size_t num_elems = 1;
