...
// Template for handlers that decode payloads, call engine, encode response.
// Each handler owns a Decoder and Encoder (cheap, stateless).
...
// --- SYNC_LOOKUP: BLOCKING, returns int ---
class SyncLookupHandler : public IRequestHandler {
 public:
  explicit SyncLookupHandler(CacheEngine& e) : engine_(e) {}
  HandlerType handler_type() const override { return HandlerType::BLOCKING; }
  std::vector<uint8_t> handle_sync(
      const std::vector<std::vector<uint8_t>>&) override {
    return {};
  ...
  std::vector<uint8_t> handle_blocking(
      const std::vector<std::vector<uint8_t>>& payloads) override {
    auto p = dec_.decode_lookup_payload(payloads);
    int result = engine_.sync_lookup(p.key, p.tp_size);
    return enc_.encode_int_response(result);
  }
...
static ServerConfig parse_args(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--host")
      ...
      cfg.max_workers = std::stoi(get_next());
      ...
      std::printf(
          ...
          "  --max-workers N         Thread pool workers (default: 4)\n"
...
int main(int argc, char* argv[]) {
  ...
  // Register all handlers (matching Python run_cache_server)
  ...
  server.add_handler(RequestType::SYNC_LOOKUP,
                     std::make_unique<SyncLookupHandler>(engine));
  ...
  // Dedicated thread pool for SYNC_LOOKUP (8 workers, same as Python)
