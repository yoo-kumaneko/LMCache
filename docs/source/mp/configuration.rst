...
Source: ``lmcache/v1/multiprocess/chunk_hash_logger.py``

When enabled, the server records chunk hashes computed during ``lookup()``
to rotating JSONL files for offline analysis.  Disabled by default.
